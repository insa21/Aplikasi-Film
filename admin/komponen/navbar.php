 <!DOCTYPE html>
 <html>

 <head>
     <meta charset="utf-8">
     <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
     <meta name="description" content="Start your development with a Dashboard for Bootstrap 4.">
     <meta name="author" content="Creative Tim">
     <title> InsaFilm | Nonton Gratis Tanpa Karcis</title>
     <!-- Favicon -->
     <link rel="icon" href="../../film-native/admin/backend/assets/img/brand/logo3.png" type="image/png">
     <!-- Fonts -->
     <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Open+Sans:300,400,600,700">
     <!-- Icons -->
     <link rel="stylesheet" href="../../film-native/admin/backend/assets/vendor/nucleo/css/nucleo.css" type="text/css">
     <link rel="stylesheet"
         href="../../film-native/admin/backend/assets/vendor/@fortawesome/fontawesome-free/css/all.min.css"
         type="text/css">
     <!-- Page plugins -->
     <!-- Argon CSS -->
     <link rel="stylesheet" href="../../film-native/admin/backend/assets/css/argon.css?v=1.1.0" type="text/css">
     <!-- Page plugins -->
     <link rel="stylesheet" href=".../../film-native/admin/backend/assets/vendor/select2/dist/css/select2.min.css">

     <!-- Datatables CSS -->
     <link rel="stylesheet"
         href="../../film-native/admin/backend/assets/vendor/datatables.net-bs4/css/dataTables.bootstrap4.min.css">
     <link rel="stylesheet"
         href="../../film-native/admin/backend/assets/vendor/datatables.net-buttons-bs4/css/buttons.bootstrap4.min.css">
     <link rel="stylesheet"
         href="../../film-native/admin/backend/assets/vendor/datatables.net-select-bs4/css/select.bootstrap4.min.css">
     <!-- Page plugins Sweet Alert -->
     <link rel="stylesheet" href="../../film-native/admin/backend/assets/vendor/animate.css/animate.min.css">
     <link rel="stylesheet" href="../../film-native/admin/backend/assets/vendor/sweetalert2/dist/sweetalert2.min.css">


 <body>
     <!-- Sidenav -->
     <nav class="sidenav navbar navbar-vertical fixed-left navbar-expand-xs navbar-light bg-white" id="sidenav-main">
         <div class="scrollbar-inner">
             <!-- Brand -->
             <div class="sidenav-header d-flex align-items-center">
                 <a class="navbar-brand" href="#">
                     <img src="../../film-native/admin/backend/assets/img/brand/logo3.png" width="100%" height="100%" class="navbar-brand-img"
                         alt="...">   
                         <!-- | InsaFilm -->
                 </a>
                 <div class="ml-auto">
                     <!-- Sidenav toggler -->
                     <div class="sidenav-toggler d-none d-xl-block" data-action="sidenav-unpin"
                         data-target="#sidenav-main">
                         <div class="sidenav-toggler-inner">
                             <i class="sidenav-toggler-line"></i>
                             <i class="sidenav-toggler-line"></i>
                             <i class="sidenav-toggler-line"></i>
                         </div>
                     </div>
                 </div>
             </div>
             <div class="navbar-inner">
                 <!-- Collapse -->
                 <div class="collapse navbar-collapse" id="sidenav-collapse-main">
                     <!-- Nav items -->
                     <ul class="navbar-nav">
                         <li class="nav-item">
                             <a class="nav-link active" href="#navbar-dashboards" data-toggle="collapse" role="button"
                                 aria-expanded="true" aria-controls="navbar-dashboards">
                                 <i class="ni ni-shop text-primary"></i>
                                 <span class="nav-link-text">Dashboards</span>
                             </a>
                            <div class="collapse show" id="navbar-dashboards">
                                <ul class="nav nav-sm flex-column">
                                <li class="nav-item">
                                        <a href="index.php" class="nav-link">Dashboard </a>
                                </li>    
                                <li class="nav-item">
                                        <a href="insert.php" class="nav-link">Tambah Film </a>
                                    </li>
                                    <li class="nav-item">
                                        <a href="lihat_data.php" class="nav-link">Lihat Data Film</a>
                                    </li>
                                </ul>
                            </div>
                         </li>
                         </li>
                         <li class="nav-item">
                             <a class="nav-link" href="calender.php">
                                 <i class="ni ni-calendar-grid-58 text-red"></i>
                                 <span class="nav-link-text">Calendar</span>
                             </a>
                         </li>
                 </div>
             </div>
         </div>
     </nav>